...
import { CheckCircle2, Truck, MapPin, BadgeCheck } from "lucide-react";
...
const MOCK_DATABASE: Record<string, TimelineStep[]> = {
  "LMX-GRA-8064": [
    {
      id: "1",
 ...
 *
 * Provides a CSN lookup form and renders a chain-of-custody timeline
 * with completed / in-progress / pending step visualizations.
 */
export function TrackingWidget() {
  const [csnQuery, setCsnQuery] = useState("LMX-GRA-8064");
  const [activeData, setActiveData] = useState<TimelineStep[] | null>(
    MOCK_DATABASE["LMX-GRA-8064"]
  );
  const [errorMessage, setErrorMessage] = useState<string | null>(null);

  /** Handle CSN search — looks up mock database. */
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    const result = MOCK_DATABASE[csnQuery.trim().toUpperCase()];
    if (result) {
      ...
      setActiveData(null);
      setErrorMessage(
        `No active consignment found matching record "${csnQuery}". Please verify your CSN.`
      );
    }
  };
    ...
    <div className="space-y-6">
        ...
        <div className="bg-white p-5 sm:p-6 md:p-8">
          <form onSubmit={handleSearch}>
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 items-stretch sm:items-center">
              ...
              <button
                type="submit"
                className="bg-brand-red-500 hover:bg-brand-red-600 text-white font-semibold px-6 sm:px-8 py-2.5 sm:py-3 rounded-lg shadow-sm whitespace-nowrap transition-colors duration-200 cursor-pointer text-sm focus-visible:ring-2 focus-visible:ring-brand-red-500 focus-visible:ring-offset-2"
              >
                Track Document
              </button>
            </div>
      ...
      {activeData && (
        <div className="bg-white border border-slate-200 rounded-xl p-5 sm:p-6 md:p-8 shadow-sm">
          <h2 className="text-lg sm:text-xl font-bold text-slate-900 border-b border-slate-200 pb-4 mb-6 sm:mb-8">
            Chain of Custody
          </h2>
