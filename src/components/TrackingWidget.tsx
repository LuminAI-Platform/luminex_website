    ...
    <div className="space-y-6">
          ...
          <form onSubmit={handleSearch}>
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center">
              <input
                ...
                className="w-full border border-slate-300 rounded-lg px-4 py-3 text-slate-900 font-mono tracking-wide placeholder-slate-400 focus:outline-hidden focus:ring-2 focus:ring-navy-900"
              />
              <button
                type="submit"
                className="bg-brand-red-500 hover:bg-brand-red-600 text-white font-semibold px-8 py-3 rounded-lg shadow-xs whitespace-nowrap transition cursor-pointer"
              >
                Track Document
              </button>
            </div>
      ...
      {activeData && (
        <div className="bg-white border border-slate-200 rounded-xl p-6 md:p-8 shadow-xs">
          <h2 className="text-xl font-bold text-navy-900 border-b border-slate-200 pb-4 mb-8">
            Chain of Custody
          </h2>
                ...
                {step.status === "in_progress" ? (
                  ...
                  <div className="bg-blue-50/40 border-t-2 border-brand-red-500 rounded-lg p-5">
                    <div className="flex items-center justify-between gap-2">
                      <h3 className="font-bold text-brand-red-600 text-base">
                        {step.title}
                      </h3>
                    ...
                    <div className="bg-white border border-blue-100 rounded-md p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-bold text-brand-red-600">{step.requiredId}</p>
                      </div>
                    </div>
                  </div>
                ) : (
                  ...
                  <div>
                    <div className="flex items-center justify-between gap-2">
                      <h3 className={`font-bold text-base ${step.status === "pending" ? "text-slate-400" : "text-navy-900"}`}>
                        {step.title}
                      </h3>
                      {step.timestamp && (
